/* 1 ~ 5까지의 원소로 이루어진 배열을 만드는 함수를 재귀함수와 TCO로 작성하시오.*/

// 재귀 함수
const arr = [];
const makeArray = (n) => {
  if (n < 1) return arr.reverse();
  arr.push(n);
...
const makeArray5 = (n) => {
  if (n === 1) return [1];
  return [...makeArray5(n - 1), n];
};

console.log(makeArray5(5)); // [1,2,3,4,5]

// TCO

const makeArray1 = (n, acc = []) => {
  if (n < 1) return acc.reverse();
  acc.push(n);
